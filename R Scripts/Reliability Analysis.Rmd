...
#T2_R2 %>% group_by(ID,bin_number) %>% summarise(min = min(bin_number), max = max(bin_number), mean_lanesd = mean(sd_speed))
 ...
 Fam_t_test <- Fam  %>% select(ID ,sd_speed) %>% group_by(ID) %>% summarise(var = mean(sd_speed))
 T1_R1_t_test <- T1_R1 %>% group_by(ID) %>% summarise(var = mean(sd_speed))
 T1_R2_t_test <- T1_R2 %>% group_by(ID) %>% summarise(var = mean(sd_speed))
 T2_R1_t_test <- T2_R1 %>% group_by(ID) %>% summarise(var = mean(sd_speed))
 T2_R2_t_test <- T2_R2 %>% group_by(ID) %>% summarise(var = mean(sd_speed))
 
 Fam_onlyT2 <- Fam %>% filter(ID == Completed_T2) %>% group_by(ID) %>% summarise(var = mean(sd_speed))
 T1_R1_onlyT2 <-T1_R1 %>% filter(ID == Completed_T2) %>% group_by(ID) %>% summarise(var = mean(sd_speed))
 T1_R2_onlyT2 <- T1_R2 %>% filter(ID == Completed_T2) %>% group_by(ID) %>% summarise(var = mean(sd_speed))
...
##Fam vs T2_R1 difference 
 Fam_vs_T1R2difference <- Fam_onlyT2$var - T2_R1_t_test$var
 Fam_vs_T1R2logdifference <- log(Fam_onlyT2$var) - log(T2_R1_t_test$var)
 shapiro.test(Fam_vs_T1R2difference)
 shapiro.test(Fam_vs_T1R2logdifference)
 ...
 ## T1_R1 vs T2_R1
 T1_R1_vs_T2_R2difference <- T1_R1_t_test$var - T2_R1_t_test$var
 T1_R1_vs_T2_R2logdifference <- log(T1_R1_t_test$var) - log(T2_R1_t_test$var)
 shapiro.test(T1_R1_vs_T2_R2difference)
 shapiro.test(T1_R1_vs_T2_R2logdifference) 
 
 t.test(T1_R1_t_test$var,T2_R1_t_test$var, paired = TRUE)
 t.test(log(T1_R1_t_test$var),log(T2_R1_t_test$var), paired = TRUE)
 psych::ICC(data_frame(T1_R1_t_test$var,T2_R1_t_test$var))
 ...
 shapiro.test(T1_R2_vs_T2_R2logdifference) 
 ...
 ##T2_R1 vs T2_R2
 T2_R1_vs_T2R2difference <- Fam_onlyT2$var - T2_R1_t_test$var
 Fam_vs_T1R2logdifference <- log(Fam_onlyT2$var) - log(T2_R1_t_test$var)
 shapiro.test(Fam_vs_T2R2difference)
 shapiro.test(Fam_vs_T2R2logdifference)
 
 t.test(T2_R1_t_test$var,T1_R2_t_test$var, paired = TRUE)
...
blandr.display.and.draw(Fam$avg_lanepos,T1_R1$avg_lanepos) + theme_minimal()
```
