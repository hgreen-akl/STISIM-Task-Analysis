...
#T2_R2 %>% group_by(ID,bin_number) %>% summarise(min = min(bin_number), max = max(bin_number), mean_lanesd = mean(sd_speed))
 ...
 Fam_t_test <- Fam   %>% group_by(ID) %>% summarise(var = mean(sd_speed))
 ...
 psych::ICC(data_frame(Fam_t_test$var, T1_R1_t_test$var))$results %>% head()
...
##Fam vs T2_R1 difference 
 ...
 ## T1_R1 vs T2_R1
 T1_R1_vs_T2_R2difference <- T1_R1_onlyT2$var - T2_R1_t_test$var
 T1_R1_vs_T2_R2logdifference <- log(T1_R1_onlyT2$var) - log(T2_R1_t_test$var)
...
blandr.display.and.draw(log10(Fam$sd_speed),log10(T1_R1$sd_speed)) + theme_bw() + ylab("Familarisation - Trial 1 Run 1")
```
