...
bin_by_dist <- function(x, bin_size = 100) {
 ...
 Task <- if_else(y$scenario[1] == "Urban", 1,2)
  ...
  bin_factors <<- as.factor(cut(y$Total_dist, breaks =  binned_data$bin_start, labels = FALSE))
...
bin_by_saccades <- function(x, bin_size = 100) {
  pat <- x
  file_to_bin <- list.files(
    path = "G:/Team Drives/Research Team/Projects/2018 Driving Sim Reproducibility/3_Raw Data/", pattern = paste0("?",pat,"_checked"), recursive = TRUE, full.names = TRUE)
