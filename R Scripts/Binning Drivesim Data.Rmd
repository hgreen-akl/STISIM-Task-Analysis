...
List_of_checked_files <- read_excel("G:/Shared Drives/Research Team/Projects/2018 Driving Sim Reproducibility/File Reference Database.xlsx") %>%  mutate(pattern_name = paste0(Participant,"_", Run_Number)) %>% dplyr::filter(CSV_checked == "Yes") 
...
file_to_import <- list.files(path = "G:/Shared drives/Projects/2018 Driving Sim Reproducibility/", pattern = paste0("?","AL_5","_checked.csv"), recursive = TRUE, full.names = TRUE)
...
bin_by_saccades <- function(x, bin_size = 100) {
 ...
 csv_to_bin %>% 
  ...
  pat <- x
  file_to_bin <- list.files(path = "G:/Shared drives/Projects/2018 Driving Sim Reproducibility/7_Filtered_data/", 
