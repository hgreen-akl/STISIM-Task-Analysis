...
```{r}
##Overall Performance
attn_speed_overall <- attention_performance_database %>% 
    group_by(ID, Session, Run, target_deviation) %>%
    summarise(mean_RT = mean(reaction_time), sd_RT = sd(reaction_time), count = n())

attn_accuracy_overall <- attention_performance_database %>% 
    group_by(ID, Session, Run, target_deviation)%>%
    summarise(mean_RT = mean(reaction_time), sd_RT = sd(reaction_time), count = n())

##Performance based on congruency
attention_performance_database %>% filter(response_type == "Correct") %>%
