...
attention_RT_boxplot <- attention_performance_database %>% 
    ...
    geom_boxplot(outlier.shape = NA, size = .6, color = "black")+
        ...
        theme_classic()+
        theme(legend.key.size = unit(2.5,"line"),strip.background = element_blank(), line = )+
        facet_wrap(~congruency)
attention_RT_boxplot

ggsave(filename = "Analysis Figures\\Attention_task_boxplot_bycongruency.eps", 
...
saccade_attn_joined <- saccade_attn_joined %>% 
                        mutate(Timepoint = factor(str_c(Session,Run, sep = "_"), levels = c("F_1", "1_1", "1_2","2_1","2_2")))
...
organised_saccade_data %>% 
    ...
    group_by(ID, Session, Run, saccade_bin) %>%
    summarise(mean_peak_velocity = mean(`peak_dva/s`), sd_peak_velocity = sd(`peak_dva/s`), mean_velocity = mean(), sd_velocity = sd())
