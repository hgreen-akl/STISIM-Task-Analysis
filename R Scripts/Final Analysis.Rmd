...
y <- KSS_data %>% select(Session,Timepoint, Participant_ID,kss_score) %>%
    group_by(Session, Timepoint) %>% 
    summarise(mean = mean(kss_score), sd = sd(kss_score),sem = sd(kss_score)/sqrt(n()))

fit <- aov(kss_score ~ Session * Timepoint, data = KSS_data)
summary(fit)
```
...
driving_perf_by_distance %>% group_by(Trial, Run, bin_number) %>% select (7:16) %>% summarise(mean = mean(7:12,15,16), sd = sd(7:12,15,16),sem = sd(7:12,15,16)/sqrt(n()))
