...
pattern_name <- "HB_C_1_31082018"
test_filename <- list.files(path = "G:\\Team Drives\\Research Team\\Projects\\2018 Driving Sim Reproducibility\\4_Analysis_Saccades\\",
...
combined_results$target_deviation <- ifelse(combined_results$condition %% 3 == 1, "10", 
                                            ifelse(combined_results$condition %% 3 == 2, "25",
                                                   ifelse(combined_results$condition %% 3 == 0, "35", NA)))
...
combined_results$saccades_to_check <- ifelse(combined_results$`correct direction` == 0, "Check", 
                                             ifelse(combined_results$difference %>% abs() >= 5, "Check", "Fine"))

saccades_to_check <- combined_results %>% 
                        filter(saccades_to_check == "Check") %>% 
                        select(`trial number`, eye, valid, `correct direction`, `dva/s`, `peak_dva/s`, difference, response_type)
