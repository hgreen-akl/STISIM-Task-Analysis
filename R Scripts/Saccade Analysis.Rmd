...
saccades_database <- saccades_database %>% filter(is.double(`peak_dva/s`)) %>% filter(`peak_dva/s` < 2000)
...
summarised <- saccades_database %>% 
    ...
    select(ID, Session, Run,`trial number`, `peak_dva/s`, `dva/s`, amplitude) %>% summarise(peak_V = mean(`peak_dva/s`), avg_V = mean(`dva/s`), amplitude = mean(amplitude))
